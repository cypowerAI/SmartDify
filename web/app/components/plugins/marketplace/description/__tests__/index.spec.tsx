...
const pluginTranslations: Record<string, string> = {
  ...
  'marketplace.difyMarketplace': 'SmartDify Marketplace',
  'marketplace.and': 'and',
...
describe('Description', () => {
  ...
  describe('Non-Chinese Locale Rendering', () => {
    ...
    it('should render SmartDify Marketplace text at the end for non-Chinese locales', () => {
      render(<Description />)

      const subheading = screen.getByRole('heading', { level: 2 })
      expect(subheading.textContent).toContain('SmartDify Marketplace')
    })
  ...
  describe('Chinese (zh-Hans) Locale Rendering', () => {
    ...
    it('should render SmartDify Marketplace text for zh-Hans locale', () => {
      render(<Description />)
    ...
    it('should render both zh-Hans specific elements and shared elements', () => {
      ...
      // zh-Hans has specific element order: "in" -> SmartDify Marketplace -> Discover
  ...
  describe('Locale Variations', () => {
    ...
    it('should handle ja-JP locale as non-Chinese', () => {
      ...
      // Should render in non-Chinese format (discover first, then "in SmartDify Marketplace" at end)
      const subheading = screen.getByRole('heading', { level: 2 })
      expect(subheading.textContent).toContain('SmartDify Marketplace')
    })
  ...
  describe('Conditional Rendering', () => {
    ...
    it('should render non-Chinese specific content when locale is not zh-Hans', () => {
      ...
      // Non-Chinese has "in" and "SmartDify Marketplace" at the end
      const subheading = screen.getByRole('heading', { level: 2 })
      expect(subheading.textContent).toContain('SmartDify Marketplace')
    })
    ...
    it('should not render zh-Hans intro content for non-Chinese locales', () => {
      ...
      // For en-US, the order should be Discover ... in SmartDify Marketplace
      // The "in" text should only appear once at the end
      const subheading = screen.getByRole('heading', { level: 2 })
      const content = subheading.textContent || ''

      // "in" should appear after "Bundles" and before "SmartDify Marketplace"
      const bundlesIndex = content.indexOf('Bundles')
      const inIndex = content.indexOf('in')
      const marketplaceIndex = content.indexOf('SmartDify Marketplace')
    ...
    it('should render zh-Hans with proper word order', () => {
      ...
      // zh-Hans order: in -> SmartDify Marketplace -> Discover -> categories
      const inIndex = content.indexOf('in')
      const marketplaceIndex = content.indexOf('SmartDify Marketplace')
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle zh-Hant as non-Chinese simplified', () => {
      ...
      // Check that "SmartDify Marketplace" appears at the end (non-Chinese format)
      const discoverIndex = content.indexOf('Discover')
      const marketplaceIndex = content.indexOf('SmartDify Marketplace')

      // For non-Chinese locales, Discover should come before SmartDify Marketplace
  ...
  describe('Content Structure', () => {
    ...
    it('should render all text elements in correct order for zh-Hans', () => {
      ...
      // zh-Hans order: in -> SmartDify Marketplace -> Discover -> categories -> and -> Bundles
      const inIndex = content.indexOf('in')
      const marketplaceIndex = content.indexOf('SmartDify Marketplace')
      const discoverIndex = content.indexOf('Discover')
...
describe('Description Integration', () => {
  ...
  it('should correctly differentiate between zh-Hans and en-US layouts', () => {
    ...
    const enMarketplaceIndex = enContent.indexOf('SmartDify Marketplace')
    const enDiscoverIndex = enContent.indexOf('Discover')
    const zhMarketplaceIndex = zhContent.indexOf('SmartDify Marketplace')
    const zhDiscoverIndex = zhContent.indexOf('Discover')

    // en-US: Discover comes before SmartDify Marketplace
    expect(enDiscoverIndex).toBeLessThan(enMarketplaceIndex)

    // zh-Hans: SmartDify Marketplace comes before Discover
