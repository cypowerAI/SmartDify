...
/**
 * Test default SmartDify branding behavior
 * When custom branding is disabled, should use "SmartDify" as the brand name
 */
describe('use default branding', () => {
   ...
   * Format: "[page] - SmartDify"
   */
  it('document title should be test-SmartDify if set title', () => {
    renderHook(() => useDocumentTitle('test'))
    expect(document.title).toBe('test - SmartDify')
  })
   ...
   * Format: "SmartDify"
   */
  it('document title should be SmartDify if not set title', () => {
    renderHook(() => useDocumentTitle(''))
    expect(document.title).toBe('SmartDify')
  })
})
