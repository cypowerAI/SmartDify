...
class AliyunLogStore:
    ...
    def __init__(self) -> None:
        ...
        # Append SmartDify identification to the existing user agent
        original_user_agent = self.client._user_agent  # pyright: ignore[reportPrivateUsage]
        dify_version = dify_config.project.version
        enhanced_user_agent = f"SmartDify,SmartDify-{dify_version},{original_user_agent}"
    ...
    def create_logstore_if_not_exist(self) -> None:
        ...
        for logstore_name in logstore_name_list:
            ...
            # Ensure index contains all SmartDify-required fields
    ...
    def _merge_index_configs(
        ...
        """
        Intelligently merge existing index config with SmartDify's required field indexes.

        This method:
        1. Preserves all existing field indexes in logstore (including custom fields)
        2. Adds missing SmartDify-required fields
        3. Updates fields where type doesn't match (with json/text compatibility)
        4. Corrects case mismatches (e.g., if SmartDify needs 'status' but logstore has 'Status')

        Type compatibility rules:
        - json and text types are considered compatible (users can manually choose either)
        - All other type mismatches will be corrected to match SmartDify requirements
        ...
        Args:
            ...
            required_keys: SmartDify's required field index configurations
        ...
        if missing_fields:
            logger.info(
                "Logstore %s: Adding %d missing SmartDify-required fields: %s",
                logstore_name,
                len(missing_fields),
    ...
    def ensure_index_config(self, logstore_name: str) -> None:
        """
        Ensure index configuration includes all SmartDify-required fields.

        This method intelligently manages index configuration:
        1. If index doesn't exist, create it with SmartDify's required fields
        2. If index exists:
           - Check if all SmartDify-required fields are present
        ...
        """
        # Get SmartDify's required field indexes
        required_keys = {}
        if logstore_name == AliyunLogStore.workflow_execution_logstore:
        ...
        if existing_config is None:
            ...
            if needs_update:
                logger.info("Logstore %s: Updating index to include SmartDify-required fields", logstore_name)
                try:
                ...
                logger.info(
                    "Logstore %s: Index already contains all %d SmartDify-required fields with correct types, "
                    "no update needed",
                    logstore_name,
                    len(required_keys),
                )
