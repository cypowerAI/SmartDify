...
class MLflowDataTrace(BaseTraceInstance):
    ...
    def workflow_trace(self, trace_info: WorkflowTraceInfo):
        ...
        # fields with sys.xyz is added by SmartDify, they are duplicate to trace_info.metadata
    ...
    def _get_node_span_type(self, node_type: str) -> str:
        """Map SmartDify node types to MLflow span types"""
        node_type_mapping = {
    ...
    def _resolve_tool_call_ids(self, messages: list[dict]):
        """
        The tool call message from SmartDify does not contain tool call ids, which is not
        ...
        for msg in messages:
            ...
            if msg["role"] == "tool":
                ...
                # assuming SmartDify runs tools sequentially
                if tool_call_ids:
