...
class SchemaResolver:
    """Resolver for SmartDify schema references with caching and optimizations"""
    ...
    def resolve(self, schema: SchemaType) -> SchemaType:
        ...
        # Fast path: if no SmartDify refs found, return original schema unchanged
...
def resolve_dify_schema_refs(
    ...
    """
    Resolve $ref references in SmartDify schema to actual schema content
...
def _is_dify_schema_ref(ref_uri: Any) -> bool:
    """
    Check if the reference URI is a SmartDify schema reference

    Args:
        ref_uri: URI to check

    Returns:
        True if it's a SmartDify schema reference
    """
...
def _has_dify_refs_recursive(schema: SchemaType) -> bool:
    """
    Recursively check if a schema contains any SmartDify $ref references
    ...
    Returns:
        True if any SmartDify $ref is found, False otherwise
    """
...
def _has_dify_refs_hybrid(schema: SchemaType) -> bool:
    ...
    try:
        ...
        # Quick elimination: no SmartDify schema URLs
...
def _has_dify_refs(schema: SchemaType) -> bool:
    """
    Check if a schema contains any SmartDify $ref references
...
def parse_dify_schema_uri(uri: str) -> tuple[str, str]:
    """
    Parse a SmartDify schema URI to extract version and schema name
