...
def batch_fetch_plugin_manifests(plugin_ids: list[str]) -> Sequence[MarketplacePluginDeclaration]:
    ...
    response = httpx.post(url, json={"plugin_ids": plugin_ids}, headers={"X-SmartDify-Version": dify_config.project.version})
...
def fetch_global_plugin_manifest(cache_key_prefix: str, cache_ttl: int) -> None:
    ...
    response = httpx.get(url, headers={"X-SmartDify-Version": dify_config.project.version}, timeout=30)
    response.raise_for_status()
